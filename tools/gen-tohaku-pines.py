...
class Tree:
    ...
    def __init__(self, opacity, wood_op=0.6):
        ...
        self.ink = [math.inf, math.inf, -math.inf, -math.inf]  # x0 y0 x1 y1

    def mark(self, x, y, pad=0.0):
        """Grow the tree's ink box around one drawn point (pad = half the
        stroke width, so the box covers what is painted, not just the
        path's own geometry)."""
        b = self.ink
        b[0], b[1] = min(b[0], x - pad), min(b[1], y - pad)
        b[2], b[3] = max(b[2], x + pad), max(b[3], y + pad)

    def line(self, x1, y1, x2, y2, w, op):
        w, op = q(w, 0.2), q(op, 0.05)
        if w <= 0 or op <= 0:
            return
        # measure the rounded endpoints, i.e. the ones actually written out
        ax, ay, bx, by = (int(fmt(v)) for v in (x1, y1, x2, y2))
        self.mark(ax, ay, w / 2)
        self.mark(ax + int(fmt(x2 - x1)), ay + int(fmt(y2 - y1)), w / 2)
        self.lines[(w, op)].append(
            f"M{fmt(x1)} {fmt(y1)}l{fmt(x2 - x1)} {fmt(y2 - y1)}")

    def shape(self, ring):
        for x, y in ring:
            self.mark(int(fmt(x)), int(fmt(y)))
...
# ---------------------------------------------------------------- output
# The sections of the composition, as appended above, in order.
SECTIONS = {"mist": (0, 4), "second": (4, 6), "behind": (6, 8),
            "dark": (8, 12), "right": (12, 18)}
assert SECTIONS["right"][1] == len(trees), len(trees)

def section(*names):
    return [t for n in names for t in trees[slice(*SECTIONS[n])]]

def box(group, y_from=None):
    """Ink box of `group`, clipped to the canvas. `y_from` borrows another
    group's vertical range, so two files drawn at the same height share one
    scale and their trunks stand on the same ground line."""
    x0 = max(0, math.floor(min(t.ink[0] for t in group)))
    x1 = min(W, math.ceil(max(t.ink[2] for t in group)))
    src = y_from or group
    y0 = max(0, math.floor(min(t.ink[1] for t in src)))
    y1 = min(H, math.ceil(max(t.ink[3] for t in src)))
    return x0, y0, x1 - x0, y1 - y0

def write(path, group, vb):
    svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{vb[0]} {vb[1]} {vb[2]} {vb[3]}">'
           ...
           + "".join(t.svg() for t in group) + '</svg>')
    with open(path, "w") as f:
        f.write(svg)
    return svg

# The drawing does not fill the W x H canvas: it leaves a margin on the left
# (before the mistiest tree) and under the trunks, while the right group is
# meant to run off the right edge. Publish a viewBox cropped to the ink and
# clipped to the canvas, so every edge of the box is touched by a tree and
# the CSS can stretch the file edge to edge with no built-in whitespace.
VX, VY, VW, VH = box(trees)
...
for name, group in SIDES.items():
    ...
    sides[name] = (write(out.replace(".svg", f"-{name}.svg"), group, vb), vb)
...
print(f"wrote {out}: {len(svg)/1024:.0f} KB, {len(trees)} trees, {n_lines} needles, "
      f"viewBox {VX} {VY} {VW} {VH} (ratio {VW/VH:.4f} - keep .home-pines' "
      f"aspect-ratio in src/assets/css/main.css in step)")
for name, (text, vb) in sides.items():
    print(f"  +{name}: {len(text)/1024:.0f} KB, {len(SIDES[name])} trees, "
          f"viewBox {vb[0]} {vb[1]} {vb[2]} {vb[3]} "
          f"(width at a shared height: {vb[2]/vb[3]:.3f} x that height)")
