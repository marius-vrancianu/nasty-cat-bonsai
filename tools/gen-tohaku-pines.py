...
A_k is not the trace's own tone, though: CONTRAST bends it first. A straight
transfer reproduces the painting faithfully and reads as a blur at backdrop
scale - sixteen shallow steps spread evenly from mist to trunk, with the mist
carrying almost as much ink as the trees. The curve pulls the pale end down
and leaves the dark end where it is, so the steps bunch where the drawing
is (crisper edges, blacker trunks) and thin out where it is only mist. Dark
mode needs this most: masked over the ink colour, a faint layer over a near-
black ground gains far more apparent weight than the same layer over paper,
which is what fogs the drawing there.
...
# Tone curve. Cumulative coverage is raised to this power before the
# incremental opacities are derived: 1 is the trace's own linear tone, and
# above 1 the pale layers thin out while the darkest (coverage 1) stays put,
# which sharpens the drawing and deepens its darks. 1.7 is as far as it goes
# before the far group's mist - the depth in the painting - drops out of the
# picture altogether; the CSS opacities in main.css are set against it and
# want revisiting if it changes.
CONTRAST = 1.7
...
def opacities(greys, washi, floor):
    ...
    bent by CONTRAST; the layer's own opacity is what that coverage needs
    on top of the layer before it."""
    out, prev = [], 0.0
    for g in greys:
